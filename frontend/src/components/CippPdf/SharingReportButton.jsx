...
export const SharingReportButton = ({ sharingData, tenantName }) => {
  ...
  const handleOpen = () => {
    ...
    setError(false)
    setPdfUrl('')
    fetch(`/api/ExecGetSharingReportPdf?tenantFilter=${encodeURIComponent(tenantFilter)}`, {
  ...
  const handleDownload = () => {
    ...
    link.href = pdfUrl
    link.download = `Sharing_Report_${(tenantName || 'report').replace(/[^a-zA-Z0-9]/g, '_')}_${new Date().toISOString().split('T')[0]}.pdf`
          ...
          {error && (
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', p: 4 }}>
              <Typography variant="body2" color="error">
                The report could not be generated. Ensure the sharing data has been synced for this tenant.
              </Typography>
            </Box>
          )}
