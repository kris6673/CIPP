...
Runs can also be queued for many users at once. Select the users on the [Users](../users/README.md) page and choose **Run BEC check**, pick the scope, and one run per user (at most fifty per request) is queued as a single job that the Queue page tracks. Each run is a separate case and shows up on the [Business Email Compromise](README.md) page and in the user's own run history as it completes.
...
| Check 10: Sign-in Locations         | The user's sign-ins in the window with the application, result, IP address, country, and city, compared against the account's assigned usage location. The card's count is the number of foreign data points found across sign-ins, rule changes, safelist changes, sharing changes, and sent mail. See [#location-analysis](bec.md#location-analysis "mention") below.                                                                                                               |
