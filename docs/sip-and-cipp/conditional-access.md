...
3. **Troubleshooting and Optimisation Skills**: Acquire skills in troubleshooting and optimising Conditional Access systems for varied client scenarios.
...
* **Fundamental Principles**: Conditional Access is not processed like firewall rules; it's evaluated as one combined policy after login, prioritising deny rules over allow rules.
* **Policy Evaluation Mechanics**: Policies are merged into a single evaluation block to determine access, emphasising the importance of strategic policy design.
...
* **Licence Requirements**: Emphasis on the necessity of appropriate licensing (e.g., P1 licences) for each user to utilise Conditional Access features.
...
* **CIPP for Policy Management**: Utilising CIPP to control and streamline Conditional Access policy implementation and management across different user scenarios.
...
**A:** The feature is actually a P1 feature and is currently active for all users who are using security defaults. This applies to anyone, even without a P1 licence, as long as security defaults are enabled​​.
...
**Q: What to do with mailboxes configured with a licence and login but used by multiple users?**
...
**A:** You need to exclude the tenant ID of the organisation. It's recommended to enter the tenant ID as it never changes, although you can also use the domain name. Additionally, using a temporary access password for the 'break glass' account is a good practice​​.
